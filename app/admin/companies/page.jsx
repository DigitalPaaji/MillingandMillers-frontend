...
const Page = () => {
  ...
  const fetchAdds = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`${base_url}/compaines/get`)
      const data = response.data
      if (data.success) {
        setAdds(data.allComp || [])
      }
    } catch (error) {
  ...
  const handleStatusToggle = async (id, currentStatus) => {
    try {
      const response = await axios.put(`${base_url}/compaines/update/${id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`${base_url}/compaines/delete/${id}`)
      const data = response.data
  ...
  return (
    <div className='px-5 py-5'>
      <TopLine title="Companies" link="/admin/companies/create" />
